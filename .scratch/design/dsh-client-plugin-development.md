...
M2-04 进行中。M2-03 基线提交为 a872cfd。当前 M2-04 已暴露 typed `sendMessage` 与 `changes` Remote；`changes` 是 bundle 自有的轻量长轮询失效通知，只传 version，避免修改 Harness 静态 forwarded-event allowlist，也不向 Client 暴露 Operation。Host `view` 保留默认 after cursor，并新增 before cursor、Activity 开关、Human ref、稳定 Task number 和 Thread Message count。Client 已支持 transient Channel/Thread ref、选择 Channel 后 Team-owned 中央页、显式 Human/Agent Message timeline、最新页/加载更早、结构化 Member-ref mention、失败同 request retry、Host commit 后刷新和 changed 后重新 pull。M2-04 功能与 REAL Host/Client composition 已完成：中央页头已复用 membership controls，REAL Agent reply 会进入显式 Message projection，Client 收到 change version 后重新 pull。仅桌面/窄屏真实浏览器布局旅程按既定计划留给 M2-06；下一实现 frontier 是 M2-05。
