...
日期：2026-08-17
状态：archived；2026-08-17 已完成。以下内容记录当时的设计基线，不定义当前实现。
...
- 实施范围合同：`spec.md`。
- 实施 tickets：`issues/`；下次会话只从 `README.md` 进入。

文档优先级：领域与 authority 冲突时以 `../../../../docs/domain-model.md` / `architecture.md` 为准；UI surface 与质量门槛以本文为准；具体 public API 事实以 reuse inventory 和 Harness 已发布类型为准；ticket 只能收窄本文，不能扩大范围。
