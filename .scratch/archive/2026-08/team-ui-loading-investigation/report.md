...
- **日期：** 2026-08-21
- **状态：** archived — 调查结论已核实，期 1+2 修复已合入；本文保留为历史分析。
...
- 调查阶段没有修改生产代码；后续实施记录见 §10。
- 调查确认了逻辑上的重复请求、无范围 invalidation、全量投影构造和同步 Inbox 扫描。
- 当时没有采集可归档、可重复的 HAR、CPU profile 或按规模耗时数据；若需要性能预算，应另建工作项并用当前实现重建 fixture。
- 调查阶段已验证：`npm test` 成功，11 个测试文件、53 个测试全部通过。
