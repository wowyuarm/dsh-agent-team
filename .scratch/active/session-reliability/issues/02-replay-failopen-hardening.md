...
- [ ] 区分错误类：上游格式拒绝（unsupported migration / closed-whitelist）→ 警告 + 跳过；`corrupt session log` 现有豁免保持；其余（IO/未知）→ 维持 throw
- [ ] 日志文案包含成员、被拒 session id、拒绝原因，供诊断
- [ ] 单测：格式拒绝 → 成员激活成功且日志含跳过记录；IO 错误 → 激活失败（现状保持）
- [ ] 与 01 的组合验证：迁移后 replay 走正常读取；人为留一个不迁 artifact 时成员仍可用
- [ ] 重复 handoff 投递引信（Iris 实测，机制见 `materials/duplicate-handoff-fuse.md`）：`reconstructMissingHandoff` 的幂等判定从「boundary 存在」扩展为「boundary 存在**或**本世代 own events 含 legacy `agent-team-context-handoff` source」（continuation 不算）；单测覆盖「纯 legacy 形状的当前世代不重建」
