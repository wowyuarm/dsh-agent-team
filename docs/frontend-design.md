...
The 「提到我」 entry is one card on the wide rail above the Workspaces section and the rail's first icon on the narrow rail; both carry the same cross-Workspace direct badge (sum of each visible Workspace's `directOnly` Inbox total), hidden at zero and reading `99+` past it. While the Inbox page stands, the card/icon carries `aria-current="page"`.
...
The page merges one direct-only Inbox call per visible Workspace (Workspace order, each slice in Host order) and never acknowledges anything: opening the page is not a read, and only the durable Thread read of an opened row clears its marker and the badge. Each row is one button with (1) a one-line breadcrumb `workspace / #channel`, plus `Task #N` when taskful, (2) the Thread's opening line capped at 120 characters straight from the Host's `previewText`, and (3) the latest mention instant through the same `formatMessageTime` formatter messages use. Clicking a row selects the row's Workspace and opens its Thread. The empty state reads 「还没有人提到你」 with the hint 「需要你知道或做决定时，成员会提到你」; loading/error/retry density matches the Channel page, and a failed background refresh keeps the rows and reports through `role="alert"`. While the page is open it subscribes once without a change scope and refetches the list on every wake; closed, the fetches stop. The badge subscribes the same way for its totals and re-pulls only totals (`limit: 1`), never lists. The badge additionally refreshes from every completed durable Thread read: the Host's changes stream deliberately never wakes on reads (a read changes no shared projection), but the read consumes this reader's mention markers.
