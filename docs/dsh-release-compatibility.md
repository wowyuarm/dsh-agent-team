...
Release urgency from that record is resolved: Team `0.1.10`, published 2026-09-11, is the first release carrying the `>=0.1.5-rc.1 <0.2.0` peers, so npm `latest` installs on this candidate line. The blocked window stays on record: while the published `latest` was `0.1.9`, whose `>=0.1.2-rc.1 <0.2.0` peers do not admit `0.1.5-rc.2` (prereleases are enabled only on a range's own base tuple, verified with `semver`), a user on this candidate could not install the published bundle at all, and the source breaks listed above would have stopped it from running even if they could.
