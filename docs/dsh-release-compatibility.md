...
A custom Session message source kind is the durable trap behind the first check: `@deepseek-ai/dsh-llm` documents `MessageSourceMap` as merge-extensible, but the released-format migration audit admits a closed, build-static list of source kinds, so a plugin that adds one writes logs that a later format generation refuses wholesale. The trap has a second half that is easy to miss: reopening the same payload under the admitted `plugin` kind is refused too, because released-format validation also pins a `plugin` source's **members** to `kind`, `plugin`, `form`, `sections`, and `summary`. A bespoke envelope field such as a version, a correlation ref, or a session pair is rejected as an unexpected member even though its kind is admitted. Both halves are fail-closed on their own terms, and the second one presents as a different error (`has unexpected member`), so a fix that only renames the kind will pass review and still refuse every logged Session.

Encode plugin semantics in the admitted shape instead: `plugin` + `form` — `snapshot` with `{ name, text }` sections for structured payload, `notice` with a `summary` for a human-readable one-liner — plus the model-facing message body, which is not constrained. Read structured payload back out of the named sections. The upstream `compact` plugin needed an extra field and obtained it by having `pluginSourceValue` special-case its plugin id, so a plugin that genuinely needs bespoke members must request that upstream rather than expect a general slot.

Cover this with a test that drives the real migration stage over the messages the package actually writes, plus the retired shapes as negative controls, so the assertion cannot silently stop testing anything. The host specs do not cover it: they build Sessions natively at the current format version and never migrate one.
...
The current certified baseline is DSH `0.1.5-rc.1`. Certification covered Typert generation, full typecheck, 499 tests (1 skipped), build, pack checks, lint, and real browser composition with published-layout installation, Remote mount, Team entry/exit, and ordinary DSH restoration.
...
This round exposed a second, wider blind spot that §3.6 now covers: every check above builds its evidence from newly created Sessions, which never traverse a released-format migration. Two shipped failures lived in that gap. First, Team declared two custom Session message source kinds (`agent-team-context-handoff`, `agent-team-context-continuation`) to carry rollover handoffs; dsh `0.1.2-rc.1` shipped no session-format packages at all, so those logs were harmless until `0.1.5-rc.1` introduced the migration chain and its closed source-kind audit began refusing them wholesale (`cannot safely transform unclassified message source`). Both producers now write the admitted `plugin` + `snapshot` shape with named sections — the handoff envelope and the checkpoint correlation ride those sections — and `packages/agent-team/tests/context-source-migration.spec.ts` drives the real migration stage over the messages the package writes, keeping the retired shapes as negative controls. The audit is fail-closed and leaves the source artifact byte-identical, so the effect is unreadable Sessions, not damaged data: affected Members show as unavailable while their durable state stays `enabled`. Second, the published `0.1.9` bundle still calls `ctx.agent`, `SessionPersistence.inspect()`, and `borrowSession()` — all removed in `0.1.5-rc.1` — so on a new install Member creation throws from the setup path even though the peer warning does not block installation and the host boots normally. Both classes are release-blocking while npm `latest` points at the candidate; §3.6 is the check that surfaces them before release.
