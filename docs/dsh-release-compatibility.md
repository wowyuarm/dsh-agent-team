...
Release urgency is unaffected by that record: the published `latest` is `0.1.9`, whose peers are `>=0.1.2-rc.1 <0.2.0`. That range does not admit `0.1.5-rc.2` — prereleases are enabled only on a range's own base tuple, verified with `semver` — so a user on this candidate cannot install the published bundle at all, and the source breaks listed above would stop it from running even if they could. Until a Team release carries the `>=0.1.5-rc.1 <0.2.0` peers, this candidate is certified but not shippable.
