...
- Member Session composer（`TeamMemberSessionComposer`，接管 `conversation.composer.bar`）：键盘合同与上述一致——Enter 提交（运行中转 steer）、Shift+Enter 换行、IME composition 期间不触发；候选菜单的 ↑↓/Enter 走 `controller.arbitrate`，Tab 与 Shift+Tab 分别为接受高亮候选与原生焦点反转（对齐 team mode 与 Channel/Thread composer）。`/compact` 命令提交走公共 Session command admission：成功（matched）后 InputMachine 的 submit-settled 清空草稿，与默认 composer 一致；未匹配/执行失败保留草稿并显示重试提示。
