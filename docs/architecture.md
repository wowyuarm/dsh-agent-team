...
- The bundle targets DSH `0.1.2-rc.1` and uses the current DSH SQLite Session schema. Old SQLite Session databases are discarded and recreated. Team nevertheless retains a narrow replay normalization for old pre-Message `occurredAt` records; do not add Team Session migration, broad compatibility reads, or fallback storage paths.
