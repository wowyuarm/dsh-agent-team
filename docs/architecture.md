...
- Agent lifecycle, JSON/SQLite replay, authorization, idempotency, and revision checks stay on the Host side. Durable unread changes may produce one bounded, coalesced Agent context notification through the public Agent safe-boundary API: direct mentions carry their Message and source, Task/Claim Activities carry a concise state transition, and ordinary unread carries only a body-free Thread-first route (with a Task overlay where present). A promotion explanation is an ordinary public Message and reaches followers through their durable Thread read batch. This notification is not a second authority and does not promise exactly-once model processing.
