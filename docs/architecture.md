...
The six tools are defined in `packages/tool-agent-team/src/index.ts`; their contract is in [`team-collaboration.md`](team-collaboration.md). They mount under the isolated `team-member` preset. The Web Client is the only Human control surface and delegates every mutation through `ctx.agentTeam` typed Remote; do not restore a slash-command adapter.

Member context self-management is Host-orchestrated on top of that preset: the `new_context` tool only validates and concludes the turn; a single `ContextManagementCoordinator` watches each Member Session's events, folds rollover intent from the durable `tool/call`+`tool/result` pair (the host-only `agentTeamContext` projection definition in `context-projection.ts` is the fold), and executes the swap through the serialized lifecycle queue — wait for true idle, retire the old generation (dispose + archive, never delete), commit the idempotent `team/member-session-rolled-over` operation, activate a fresh Session, then deliver the handoff via steer and any carried non-Team input via followup. The admission gate (`agent/turn-stopping` + `agent/pre-step`) is armed only for the outgoing generation's Agent instance so later input cannot open another old-generation model request. Process state is reconstructible: pending intent, captured input, and the swap resume from the durable log after a crash.
