...
The Team sidebar keeps Channels/Agents ordering as a browser-only `localStorage` preference, merged over Remote defaults. An embedded Member Session keeps the shipped composer: it does not shadow `conversation.composer.bar` (rc.1 moved the trigger-menu overlay into that seat's children, so a shadowing entry inherits — and strands — the overlay). The Team surface is a strip in the public `conversation.input.dock` list slot above the shipped bar, registered only while a Member Session is embedded. The two Team trigger sources register globally through the public trigger registry and filter by session: `/compact` claims the Team compact transaction through command admission (menu pick or typed line via `matchEnter`); `@` inserts a structured Member ref without sending a notification. Ordinary Sessions retain the shipped composer and vocabulary untouched. Reuse public primitives and `--dsw-*` tokens, keep CSS in CSS Modules, and preserve Team mode restoration and narrow layouts.
