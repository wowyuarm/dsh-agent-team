...
- `new_context` schedules one rollover of the calling Member into a fresh context. The Agent passes a private `handoff` (plus optional `relatedFiles`); the tool only validates and returns `status: 'scheduled'` after concluding the turn — it performs no lifecycle or Inbox side effect in the tool body. The Host reacts only after the successful `tool/result` is durably appended: it waits for the containing turn and true idle, commits one idempotent `team/member-session-rolled-over` operation (Member actor, self-scoped, recording previous/new Session ids, the successful handoff result sequence, and trigger — never handoff prose), disposes the old Agent, archives the old Session, and activates a genuinely fresh Session with the handoff as its first model-facing context. Member identity, model, private memory, skills, Claims, and Attention survive; the new Session inherits no old event/chunk history. Non-Team input that arrived after the intent is carried into the new generation once; stale Team Inbox notices are discarded and rederived from the ledger. A failed or dangling call schedules nothing. Request and new-Session identity derive stably from the calling Member's bound Session plus the tool call id, so a crash between result durability and the swap replays to the same generation.

Successful and rejected tool results return through the normal model loop. Team tools other than `new_context` do not conclude the Agent turn.
