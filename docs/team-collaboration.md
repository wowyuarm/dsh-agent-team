...
「从全新上下文开始」是第三个入口：仅对 `presence === 'available'`（在线且空闲）的 enabled Member 可用，非 available 的 presence 灰掉并按各自状态给出原因（working 提示等当前 turn 结束，error/unavailable 提示先恢复在线），避免截断当前 loop。确认框点名成员并说明影响范围——仅该成员自身的旧会话日志会被移除且无法恢复，身份、私有记忆与 notes、频道和会话绑定均保持不变，后续协作从新上下文继续累积——确认后 Host 记录一条 `team/member-context-cleared` audit 操作（不改变投影：identity、私有记忆、Workspace 绑定与 sessionId 全部保留），丢弃该 Session 的持久化日志（仅 JSONL 每 session 一文件布局），并用同一 sessionId 重新走激活路径，下一次 turn 从空上下文开始；agent idle 翻转会像 running 一样广播 workspace change，让侧栏 presence 门禁在 turn 结束后即时解除。若该 Member Session 正嵌在右栏，Client 先退出嵌入视图并重开 Human 的 return 会话——dispose 后 resident 实例永久携带 `removed` 位且无公开的单会话重建入口，只有真实的 stage 移动才会 sweep 掉冻结实例——再拉取 list baseline 并重进成员会话，公开路径下的惰性重建让右栏落到空会话 hero。Human 没有 return 会话时无 stage 移动可用，右栏保持冻结视图直到下一次 stage 移动或页面刷新收敛（已记录的 Harness 公开能力限制）。
