...
每个成功或被拒绝的 Team tool result 都通过正常 model loop 返回。`new_context` 与 `context_checkpoint` 会结束 Agent turn——换窗后不得再接旧代工作，checkpoint 在其所属 turn 结束时 resolve；其余 Team tools 不结束 turn，Agent 自行决定继续读取、重试、开展项目工作、发送协作更新或结束。
