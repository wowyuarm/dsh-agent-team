...
3. Harness：
   - `docs/architecture.zh.md`：插件平面、Service Definition/Provider/Consumer 与 agent-loop 边界；
...
- 本项目：先读目标组件和 `*.module.css`，再读 `docs/architecture.zh.md` 与 `docs/development.zh.md`；需要历史视觉审计或 public UI reuse 清单时，查 `.scratch/archive/2026-08/ui-redesign/{design,research}/`。当前行为以组件源码和测试为准。
...
- 本文是查阅路线，不复制 package manifest、命令清单或领域 spec；这些事实仍由源文件和 `docs/development.zh.md` / `docs/architecture.zh.md` 负责。
