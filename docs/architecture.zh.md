...
- 嵌入的 Team Member Session 保留 shipped composer：不再接管 `conversation.composer.bar` seat（rc.1 把 trigger-menu overlay 移进了该 seat 的 children，接管者会一并继承渲染义务并使 overlay 悬空）。Team 表面改为公开 `conversation.input.dock` list slot 中、shipped bar 上方的一条提示 strip，仅在 Member Session 嵌入期间注册。两个 Team trigger sources 通过公开 trigger registry 全局注册并按 session 过滤：`/compact` 经 command admission 认领 Team compact 事务（菜单选择或整行输入经 `matchEnter`）；`@` 从当前 Workspace 插入 structured stable Member ref，不发送 notification。普通 Session 继续使用 shipped composer 以及原有 command/reference vocabulary，完全不受影响。成员会话的输入面即 shipped composer 全量能力（含成员 agent 自身的命令面与附件）。
