...
六个 model-facing tools 定义在 `packages/tool-agent-team/src/index.ts`；实现的 collaboration contract 记录在 [`team-collaboration.zh.md`](team-collaboration.zh.md)。它们由 `packages/agent-team/preset/team-member/` 下的 `team-member` preset 挂载，并位于 `cordis.patch.yml` 的 isolated scope 中。不要为了让测试可用就把 tool package 作为 global row 添加；普通 Sessions 必须保持 Team-free。

Member 上下文自主管理在这些 preset 之上由 Host 编排：`new_context` 工具只校验并结束 turn；单一 `ContextManagementCoordinator` 监听各 Member Session 的事件，从 durable 的 `tool/call`+`tool/result` pair 折出 rollover 意图（`context-projection.ts` 中 host-only 的 `agentTeamContext` projection definition 即该折叠），再经串行 lifecycle queue 执行换窗——等真正 idle、retire 旧代（dispose + 归档、绝不删除）、提交幂等的 `team/member-session-rolled-over` operation、激活全新 Session，随后以 steer 送达 handoff、以 followup 投递携带的非 Team 输入。admission gate（`agent/turn-stopping` + `agent/pre-step`）只对旧代 Agent instance 生效，使后续输入无法再开启旧代 model request。进程内状态可重建：pending 意图、捕获的输入和换窗进度在崩溃后都从 durable log 恢复。
