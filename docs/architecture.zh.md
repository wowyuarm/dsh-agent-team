...
- Client invalidation 采用有范围且定向的方式。`changes()` waiters 声明一个 scope（workspace/channel/thread）、或省略 scope 以观察任意 Team 提交，并携带可 abort 的 signal；commit 只唤醒匹配的 scopes，省略 scope 则任意提交都会唤醒。Thread read 不会唤醒任何 waiter，因为它不改变共享 projection；`team/dm-sent` direct message 是同类非事件：durable record 不改变任何共享 projection，同样不唤醒 waiter。每次 commit 后，Host 只为可能受该 operation 影响的 Members 重算 Inbox hints。Client 通过 `TeamChangeStream` 为每个 scope 共享一条可 abort 的 long-poll，而不是每页各自轮询；Human mention 徽标只订一次无 scope 的 poll，唤醒后重拉各 Workspace 的 direct-only Inbox 合计。这些只是 transport 优化，绝不是 ledger facts 的第二权威。
