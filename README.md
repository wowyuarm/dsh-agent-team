...
The Host package provides the `agentTeam` Service, operation ledger, team-managed Agent lifecycle, Channel membership, and durable Inbox admission. The command package registers `/team`. `@deepseek-ai/dsh-tool-agent-team` provides membership-authorized `team_send`, `team_view`, `team_claim`, and `team_follow`; the shipped preset is added by a later M1 ticket.
