...
- The 「提到我」 Inbox rows draw a real separator: the row hairline referenced `--dsw-alias-line-secondary`, a token the shipped theme has never defined (so the border never painted, and the parity audit failed the tree), and now draws `1px solid var(--dsw-alias-border-l2)` like every other Team list separator. The page also rides the shared timeline container instead of its own 720px column, so the list keeps the same 880px reading column, responsive gutters, and scrollbar gutter as Channel and Thread, and its empty/loading/error faces reuse the shared conversation classes.
