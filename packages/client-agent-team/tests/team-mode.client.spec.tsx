...
describe('rendered Team mode composition', () => {
  ...
  it('creates a Channel atomically with selected available Members and manages committed membership', async () => {
    ...
    const manager = b.view.getByRole('dialog', { name: 'Channel 成员' })
  ...
  it('opens a selected Channel in the Team center and sends only after Host commit', async () => {
    ...
    const pageManager = b.view.getByRole('dialog', { name: 'Channel 成员' })
    ...
    fireEvent.click(b.view.getByRole('button', { name: '添加提及' }))
    fireEvent.click(b.view.getByRole('menuitem', { name: '@builder' }))
    expect(b.view.getByRole('button', { name: '移除 @builder' })).toBeTruthy()
