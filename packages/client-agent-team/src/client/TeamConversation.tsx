...
export function TeamConversation({ t, useWorkspaces, navigation, loadChannels, loadChanges, loadMembers, sendMessage, selectThread }: TeamConversationProps) {
  ...
  if (current !== undefined && navigationState.channelRef !== undefined) {
    return <TeamChannelPage key={navigationState.channelRef} workspaceId={current.workspaceId} channelRef={navigationState.channelRef} loadChannels={loadChannels} loadChanges={loadChanges} loadMembers={loadMembers} sendMessage={sendMessage} selectThread={selectThread} t={t} />
  }
