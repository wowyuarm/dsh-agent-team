...
Member reply 必须携带准确的当前 Thread revision，并在一个 operation 内更新 Message、Follow、Delivery 和 Thread facts。Task 的 resolution 为 `open`、`accepted` 或 `closed` 都不阻止 reply；reply 只推进 Thread revision，并保留 Task 当前的 status 与 resolution。显式 follow/unfollow 是有序 Activity。结构化 mention 指向 unfollowed Member 时，必须先取得 process-local one-use confirmation token，首次调用不提交 operation；确认成功后重新建立 Follow。claim/done/release 是有序的 host-authored Activity。Active Claim 只排斥相同的 normalized Direction；不同 Direction 可以并行。显式 Human resolution 下 Task 为 `closed` 或 `done`；否则存在 active Claim 时为 `in_progress`，没有 active 但至少一个 done 时为 `in_review`，其余为 `todo`。Close 在同一 operation 内 release active Claims；reopen 保留 Claim 历史并重新派生状态。Task resolution 约束 Task/Claim mutation，不关闭 Thread 对话。Member remove 先原子标记 inactive、release owned active Claims、清除 Follows、cancel queued Deliveries，再由 Host 等待 Agent 静止并归档 session。Message 与 Activity facts 共用一个有界 sequence cursor。
