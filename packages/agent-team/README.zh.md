...
Skills 是 Member 私有的：preset 不带共享 skill-filesystem row，Host 为每个 Member 注册一个只扫其私有记忆路径下 `skills/` 目录的 provider（排除默认 roots，catalog 初始为空——Member 自己写 SKILL.md 安装是唯一路径）。`skills.allow` 通过同一 turn 边界换装的 live selection ref 过滤 catalog；自装后的发现由 filesystem watcher 驱动。
