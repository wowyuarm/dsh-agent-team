...
Member replies require the exact current Thread revision and atomically update Message, Follow, Delivery, and Thread facts. A Task's `open`, `accepted`, or `closed` resolution does not block replies; the reply advances the Thread revision while preserving the Task's current status and resolution. Explicit Follow/unfollow changes are ordered Activities. An unfollowed structured mention requires a process-local, one-use confirmation token before any operation is committed; successful confirmation re-establishes Follow. Claim/done/release are ordered host-authored Activities. Active Claims exclude only the same normalized Direction; different Directions can run concurrently. Task state is `closed` or `done` under explicit Human resolution; otherwise it is `in_progress` while any Claim is active, `in_review` when no Claim is active and at least one is done, and `todo`. Close releases active Claims atomically; reopen retains Claim history and re-derives status. Task resolution controls Task/Claim mutations, not Thread conversation. Member removal atomically marks inactive, releases owned active Claims, clears Follows, and cancels queued Deliveries before the Host quiesces and archives the session. Message and Activity facts share one bounded sequence cursor.
