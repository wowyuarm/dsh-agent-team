...
Member replies require the exact current Thread revision and atomically update Message, Follow, Delivery, and Thread facts. Explicit Follow/unfollow changes are ordered Activities. An unfollowed structured mention requires a process-local, one-use confirmation token before any operation is committed; successful confirmation re-establishes Follow. Claim/done/release are ordered host-authored Activities. Active Claims exclude only the same normalized Direction; different Directions can run concurrently. Task state is `in_progress` while any Claim is active, `in_review` when no Claim is active and at least one is done, and otherwise `todo`. Message and Activity facts share one bounded sequence cursor.
